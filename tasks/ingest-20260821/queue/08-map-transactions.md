# TASK — map-transactions

Write ONE file: `/home/danman60/projects/uvalux-platform/packages/db/scripts/salon-ingest/etl/map-transactions.ts`

Pure mappers: `transactions.csv` → Bask `SaleInput[]`, and `transaction_items.csv`
→ Bask `SaleLineInput[]`. Two exports. Import from `./contract`.
...
Import from `./contract`: `remapId`, `num`, `parseDate`, `tenderType`, type `SaleInput`, type `SaleLineInput`.
...
Export `mapSaleLines(itemRows, txRows)` — it needs the transactions to resolve
each line's salon, tender, customer and timestamp:
...
Real transaction columns: `transaction_id, visit_id, customer_id, salon_id, staff_id, transaction_at, service_revenue, retail_revenue, discount_amount, total_revenue, payment_method, membership_customer`. Item columns: `transaction_item_id, transaction_id, salon_id, item_type, product_id, service_type, quantity, unit_price, unit_cost` (item_type is `service` or `retail`).
...
- Acceptance: `tsc --noEmit` clean; BOTH `mapSales` and `mapSaleLines` exported.
