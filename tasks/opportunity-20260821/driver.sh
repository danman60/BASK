...
BASE="$REPO/tasks/opportunity-20260821"
QUEUE="$BASE/queue"
DONE="$BASE/done"
FAILED="$BASE/failed"
...
STRAY_IGNORE='node_modules|\.next|generated/|runlogs/|tasks/opportunity-20260821/'
...
# Fixture module in packages/core: literal deterministic data, typed, no I/O.
gate_fixture() {
  local rel="$1" task="$2" abs="$REPO/$1" errs=0
  [[ -s "$abs" ]] || { echo "gate: missing or empty"; return 1; }
  grep -q 'export const' "$abs" || { echo "gate: exports nothing"; errs=1; }
  grep -q "from './types'" "$abs" || { echo "gate: does not import the shared types"; errs=1; }
  grep -qE 'Date\.now\(\)|new Date\(\)' "$abs" && { echo "gate: reads the clock"; errs=1; }
  grep -q 'Math.random' "$abs" && { echo "gate: nondeterministic"; errs=1; }
  grep -qE ':\s*any\b' "$abs" && { echo "gate: uses any"; errs=1; }
  grep -qE 'await |fetch\(|prisma|db\.' "$abs" && { echo "gate: does I/O — fixtures are literal"; errs=1; }
  [[ $(wc -l < "$abs") -ge 80 ]] || { echo "gate: too short to be the dataset"; errs=1; }
  case "$task" in
    18-opportunity-fixtures)
      grep -q 'DEMO_OPPORTUNITIES' "$abs" || { echo "gate: missing DEMO_OPPORTUNITIES"; errs=1; }
      grep -q 'DEMO_OUTCOMES' "$abs" || { echo "gate: missing DEMO_OUTCOMES"; errs=1; }
      for k in "'sms'" "'email'" "'social'" "'staff_task'" "'front_desk_script'" "'staff_challenge'" "'uvalux_order'" "'coaching_request'"; do
        grep -q "kind: $k" "$abs" || { echo "gate: no action of kind $k"; errs=1; }
      done ;;
    19-monitor-fixtures)
      grep -q 'DEMO_MONITOR' "$abs" || { echo "gate: missing DEMO_MONITOR"; errs=1; }
      grep -q 'Front Desk Listener' "$abs" || { echo "gate: wrong device name"; errs=1; }
      for who in Maya Jordan Priya Tess; do
        grep -q "$who" "$abs" || { echo "gate: staff member $who missing"; errs=1; }
      done
      grep -qE "customerLabel: '[A-Z][a-z]+ [A-Z][a-z]+'" "$abs" \
        && { echo "gate: customerLabel looks like a real name"; errs=1; } ;;
  esac
  tsc_attributed "packages/core" "$rel" || errs=1
  return $errs
}
...
# A shell script destined for the infra repo. It must be syntactically valid and
# it must not be able to harm the SOURCE database it reads from.
gate_shell() {
  local f="$REPO/$1" errs=0
  [[ -s "$f" ]] || { echo "gate: missing or empty"; return 1; }
  head -c 12 "$f" | grep -q '#!/bin/bash' || { echo "gate: no shebang"; errs=1; }
  bash -n "$f" 2>/dev/null || { echo "gate: bash syntax error"; errs=1; }
  grep -q 'set -uo pipefail' "$f" || { echo "gate: no strict mode"; errs=1; }
  grep -q -- '--schema=bask' "$f" || { echo "gate: does not scope the dump to bask"; errs=1; }
  # A DROP is only ever legal against the target, under --refresh. Any DROP on
  # the same line as the source URL variable is a hard fail.
  grep -iE 'DROP .*(SRC|SOURCE)' "$f" && { echo "gate: DROP aimed at the source"; errs=1; }
  grep -q 'pg_dump' "$f" || { echo "gate: no dump step"; errs=1; }
  # verification must be row counts, not an exit code
  grep -qiE 'count\(\*\)|row_count|rowcount' "$f" || { echo "gate: no row-count verification"; errs=1; }
  return $errs
}
...
gate_md() {
  ...
  case "$task" in
    ...
    20-vo-script-v3)
      local beats; beats=$(grep -c '^### Beat' "$f")
      [[ "$beats" -eq 10 ]] || { echo "gate: $beats beats, expected exactly 10"; errs=1; }
      grep -q 'DRAFT — generated by the build queue' "$f" || { echo "gate: missing DRAFT warning"; errs=1; }
      grep -q 'Total estimated read' "$f" || { echo "gate: missing total read time"; errs=1; }
      grep -qi 'built for UVALUX' "$f" && { echo "gate: overclaims a UVALUX partnership"; errs=1; }
      grep -qi 'salon management' "$f" && { echo "gate: pitches salon management — Nick ruled it out"; errs=1; }
      for s in 'ElevenLabs' 'intelligence' 'one click' 'Monitor'; do
        grep -qi "$s" "$f" || { echo "gate: missing required element '$s'"; errs=1; }
      done ;;
    21-shot-plan-v3)
      local beats; beats=$(grep -c '^## Beat' "$f")
      [[ "$beats" -eq 10 ]] || { echo "gate: $beats beats, expected exactly 10"; errs=1; }
      grep -q 'PLAN — generated by the build queue' "$f" || { echo "gate: missing PLAN warning"; errs=1; }
      local tex; tex=$(grep -c 'Textures needed:' "$f")
      [[ "$tex" -ge 10 ]] || { echo "gate: only $tex beats name their textures"; errs=1; }
      grep -qi 'built for UVALUX' "$f" && { echo "gate: overclaims a UVALUX partnership"; errs=1; }
      for s in '/monitor' 'Opportunity' 'customers' 'peers' 'capture' 'caption' ; do
        grep -qi "$s" "$f" || { echo "gate: missing required element '$s'"; errs=1; }
      done ;;
  esac
  return $errs
...
run_gate() {
  ...
  case "$kind" in
    ...
    fixture) gate_fixture "$target" "$task" ;;
    sql)    gate_sql    "$target" ;;
    shell)  gate_shell  "$target" ;;
