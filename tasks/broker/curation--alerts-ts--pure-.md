# curation: alerts.ts (pure)

## What to build

A pure module. Import every type and helper you need from the sibling module ./types using a relative import - that file is the contract and it sits next to this one. Export one function taking a readonly array of Claim and returning an array of CurationAlert. No other imports at all, no IO, no React. Implement all seven kinds in ALERT_KINDS. thin_topic fires per category when that category has fewer than five claims whose reviewState is verified; its nodeId is the string topic then a colon then the category. single_source fires per claim when distinctEvents equals one and specificity is concrete. unanchored_attribution fires when any provenance entry has titleConfidence interpolated. orphan fires when every provenance entry has a null sessionTitle. provenance_drift fires when any provenance entry has quoteVerified false, severity blocking. stale fires when reviewState is verified and reviewedAt is more than twelve months old. For contradiction return nothing, because detecting opposing sentiment needs a model and this module is pure - leave a comment saying exactly that. Severity is blocking for provenance_drift, attention for thin_topic single_source unanchored_attribution, info otherwise. Each message is one plain grade-seven sentence.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/packages/core/src/knowledge/curation/alerts.ts`
