# walk: Compass Coaching + Knowledge

## What to build

Write a markdown QA checklist section following the exemplar's structure for two UVALUX-internal screens: slash compass slash coaching and slash compass slash knowledge. Every URL needs the query parameter role equals uvalux_rep. For coaching check the page renders and that any cited answer shows its source. For knowledge check the claims table renders with readable claim text, that filter chips are grouped under the labels State Topic and Lens, that the header leads with the number of claims still to review, that selecting a row fills the inspector with a verbatim quote shown in full, that the speaker is never shown as a name when it is unknown, and that switching to the Map view renders a 3D graph with a legend. Note that the command palette and audio playback are NOT built and must be reported SKIP. End with a section listing what to report as SKIP if unimplemented.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/tests/agent/walkthrough/06-compass-coaching-knowledge.md`
...
1. `/home/danman60/projects/uvalux-platform/tests/agent/walkthrough/06-compass-coaching-knowledge.md` exists and is complete.
2. `bash -c 'test -s /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/06-compass-coaching-knowledge.md && grep -q "^## " /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/06-compass-coaching-knowledge.md' && PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.vocab /home/danman60/projects/uvalux-platform/tests/agent/compass-knowledge.md /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/06-compass-coaching-knowledge.md` passes with exit code 0.
