# knowledge: ConfidenceBadge

## What to build

A badge communicating how trustworthy a claim's attribution is. Props: a confidence number between 0 and 1, and a titleConfidence which is either anchored or interpolated. When titleConfidence is interpolated the badge must say in plain grade-seven English that the speaker was inferred from the agenda rather than heard on the recording - this is a trust disclosure and must never be hidden or shortened to a symbol. When confidence is zero, say the quote could not be matched to the transcript. Follow the exemplar's ConsentBadge for structure and tone.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ConfidenceBadge.tsx`
...
1. `/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ConfidenceBadge.tsx` exists and exports the component/function described above.
2. It imports what it uses from `@/lib/contract`.
3. `npx tsc --noEmit && python3 -m broker.vocab /home/danman60/projects/uvalux-platform/apps/web/src/components/compass/primitives.tsx /home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ConfidenceBadge.tsx --contract /home/danman60/projects/uvalux-platform/packages/core/src/knowledge/curation/types.ts` passes with exit code 0.
