# knowledge: ClaimFilterBar

## What to build

A horizontal bar of filter chips above the claims table. Import ClaimFilters, REVIEW_STATES, REVIEW_STATE_LABEL and CLAIM_CATEGORIES from the package @bask/core. Props: filters of type ClaimFilters and onChange taking a ClaimFilters. The fields on ClaimFilters are named exactly reviewState, category and lens - each an optional array. Do not invent plural names like reviewStates or categories. Render three chip groups: review state over REVIEW_STATES labelled via REVIEW_STATE_LABEL, category over CLAIM_CATEGORIES, and lens over the literal strings advice recall marketing. Each chip is a button with className cp-chip, plus cp-chip--on when its value is present in the matching array; clicking toggles that value and calls onChange with a new filters object. Add a clear-all button rendered only when at least one filter is active. Wrap everything in a div with className cp-filterbar. Hold no internal state.
