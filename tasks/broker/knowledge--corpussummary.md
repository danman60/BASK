# knowledge: CorpusSummary

## What to build

A compact header strip above the claims table. Props: corpusName string, total number, decided number, alertCount number. Show the corpus name and the counts in an inline row, and include a progress bar by importing ReviewProgressBar from ./ReviewProgressBar and passing it decided and total. Always show absolute numbers beside any percentage - never a bare percentage. When total is zero, show a sentence saying the corpus is empty and that it loads via the load-claims script, and render no bar. Wrap in a div with className cp-corpus-summary.
