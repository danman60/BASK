# knowledge: ClaimInspector

## What to build

The right-hand panel showing one selected claim. Import Claim from the package @bask/core. Props: claim which is a Claim or null, onVerify, onReject, and an optional onPlay. When claim is null render a short empty state paragraph telling the person to pick a row. Otherwise render in this order: ReviewStateChip imported from ./ReviewStateChip showing claim.reviewState, then the verbatim quote as a blockquote with className cp-quote shown in full at any length and never shortened, then the claim sentence in a p with className cp-claim-sentence, then a div with className cp-provenance containing one ProvenanceCard imported from ./ProvenanceCard per entry in claim.provenance passing the entry and onPlay, then two buttons with className cp-btn calling onVerify and onReject.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ClaimInspector.tsx`
