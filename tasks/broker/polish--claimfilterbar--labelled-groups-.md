# polish: ClaimFilterBar (labelled groups)

## What to build

A filter bar. Import ClaimFilters, REVIEW_STATES, REVIEW_STATE_LABEL and CLAIM_CATEGORIES from the package @bask/core. Props: filters and onChange. The ClaimFilters fields are named exactly reviewState, category and lens, each an optional array - do not invent plural names. Render THREE LABELLED GROUPS, each a div with className cp-filtergroup containing a span with className cp-filtergroup-label holding the group name, followed by its chips. Group one is labelled State over REVIEW_STATES using REVIEW_STATE_LABEL. Group two is labelled Topic over CLAIM_CATEGORIES. Group three is labelled Lens with exactly three chips whose VALUES are advice, recall and marketing but whose visible LABELS are Advice, War stories and Voice-of-customer respectively - the word marketing must never appear as a lens label because it collides with the marketing topic chip in group two. Each chip is a button className cp-chip plus cp-chip--on when active, toggling its value and calling onChange with a new object. A clear-all button className cp-chip cp-chip--clear appears only when a filter is active. Wrap all groups in a div className cp-filterbar. Hold no state.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ClaimFilterBar.tsx`
...
1. `/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ClaimFilterBar.tsx` exists and is complete.
2. It imports what it uses from `@bask/core`.
3. `npx tsc --noEmit -p apps/web/tsconfig.json && PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.vocab /home/danman60/projects/uvalux-platform/apps/web/src/components/compass/primitives.tsx /home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ClaimFilterBar.tsx --contract /home/danman60/projects/uvalux-platform/packages/core/src/knowledge/curation/types.ts` passes with exit code 0.
