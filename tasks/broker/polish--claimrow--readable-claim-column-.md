# polish: ClaimRow (readable claim column)

## What to build

A row in the claims table. Import Claim and formatTimecode from the package @bask/core. Props: claim, focused boolean, onSelect, onVerify, onReject. THE CLAIM TEXT IS THE POINT OF THIS ROW - it must be readable, not squeezed. Render exactly four cells in this order and no more. First a narrow state cell containing only a span with className cp-state-dot plus a second class cp-state-dot--- followed by the claim reviewState value, and a visually hidden span with the state text for screen readers. Second the claim text in a td with className cp-claim-cell, wrapped in a span with className cp-claim-text. Third the category in a td with className cp-topic-cell. Fourth the timecode of the first provenance entry via formatTimecode in a td with className cp-at-cell, guarding against an empty provenance array. Do NOT render moment and do NOT render a sources count - both moved to the inspector. The tr gets className cp-claim-row and additionally cp-claim-row--focused when focused, with onClick calling onSelect. No colours in the component.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ClaimRow.tsx`
