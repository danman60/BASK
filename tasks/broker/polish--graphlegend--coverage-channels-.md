# polish: GraphLegend (coverage channels)

## What to build

A compact static legend for the 3D map. No props, no state. It explains the channels as they now mean: node SIZE is how many claims a topic holds, node COLOUR is the share of those claims a human has verified, node BRIGHTNESS is provenance strength, and a ring means the node has an open alert. Render a div className cp-legend containing span elements className cp-legend-item. The first four items each begin with an i element className cp-legend-dot plus a second class of cp-legend-dot--none, cp-legend-dot--some, cp-legend-dot--most and cp-legend-dot--all respectively, labelled None verified, Some verified, Mostly verified and All verified. Then a span className cp-legend-sep with aria-hidden true. Then three more items with the plain text Bigger = more claims, Brighter = stronger provenance, and Sparse and dim = thin topic. No colours in the component; the dot classes carry them.
