# walk: Monitor + Data sharing + Book

## What to build

Write a markdown QA checklist section following the exemplar's structure for three screens: slash monitor which is the Front Desk Monitor, slash settings slash data-sharing which is the consent screen, and slash book which is the customer-facing booking page. For Monitor check the listener tile, scored conversations, coaching patterns and the team table all render, and check the consent pledge is visible - note that Monitor renders from fixtures and captures no real audio, so the tester must confirm nothing implies live recording. For data-sharing check the consent tiers are explained in plain language and that the page states what UVALUX can and cannot see. For book check the page renders for a customer with no login. End with a section listing what to report as SKIP if unimplemented.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/tests/agent/walkthrough/04-monitor-consent-book.md`
...
1. `/home/danman60/projects/uvalux-platform/tests/agent/walkthrough/04-monitor-consent-book.md` exists and is complete.
2. `bash -c 'test -s /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/04-monitor-consent-book.md && grep -q "^## " /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/04-monitor-consent-book.md' && PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.vocab /home/danman60/projects/uvalux-platform/tests/agent/compass-knowledge.md /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/04-monitor-consent-book.md` passes with exit code 0.
