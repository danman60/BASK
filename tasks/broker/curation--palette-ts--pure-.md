# curation: palette.ts (pure)

## What to build

A pure module. Import every type you need from the sibling module ./types using a relative import - that file is the contract and sits next to this one. Do not import anything else. Export buildPaletteIndex taking a readonly array of Claim and returning PaletteItem values: one per claim, plus one per distinct category, moment, speaker and session. For a claim the label is its claim text, the hint is the first provenance entry knowledgeRef, focusNodeId is the claim id. For grouping items focusNodeId is the kind then a colon then the value. Also export filterPalette taking the items and a query string and returning ranked matches. Matching is subsequence based and case insensitive so typing mem matches memberships. Rank higher when matched characters are contiguous and when the match starts at a word boundary. An empty query returns every item unchanged.
