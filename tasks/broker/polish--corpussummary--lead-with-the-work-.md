# polish: CorpusSummary (lead with the work)

## What to build

A header strip. Props: corpusName string, total number, decided number, alertCount number, and an optional onJumpToNext callback. LEAD WITH THE REMAINING WORK, not the total: the largest text is the number still to review, which is total minus decided, followed by the words to review. Render that in a p with className cp-corpus-lead, with the number itself in a strong with className cp-corpus-count. Beneath it in a p with className cp-note give the supporting counts, total claims and decided, as absolute numbers. Import ReviewProgressBar from ./ReviewProgressBar and render it after that, passing decided and total. When onJumpToNext is provided render a button className cp-btn with the text Next unreviewed that calls it. When total is zero show instead a single sentence saying the corpus is empty and loads via the load-claims script, and render no bar and no button. Wrap in a div className cp-corpus-summary.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/CorpusSummary.tsx`
