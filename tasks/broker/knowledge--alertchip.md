...
A chip for one curation alert. Props: the alert kind and the severity, both from the contract, plus an optional count. Render the label from ALERT_LABEL. Severity drives a modifier class only - info, attention, blocking. When count is given and above one, append it in parentheses. Seven alert kinds exist; all must map.
...
1. `/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/AlertChip.tsx` exists and exports the component/function described above.
2. It imports what it uses from `@/lib/contract`.
3. `npx tsc --noEmit && python3 -m broker.vocab /home/danman60/projects/uvalux-platform/apps/web/src/components/compass/primitives.tsx /home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/AlertChip.tsx --contract /home/danman60/projects/uvalux-platform/packages/core/src/knowledge/curation/types.ts` passes with exit code 0.
