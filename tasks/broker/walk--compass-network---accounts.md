# walk: Compass Network + Accounts

## What to build

Write a markdown QA checklist section following the exemplar's structure for the UVALUX-internal Compass screens: slash compass which is the Call List, slash compass slash network, slash compass slash accounts, and an account detail page at slash compass slash accounts slash a slug. IMPORTANT: every Compass route needs the query parameter role equals uvalux_rep or it returns a forbidden error, so the checklist must say to append that to every URL. Check the network map renders, that health bands expand to show the factors behind them, and that no raw health score is ever displayed - the consent filter deliberately keeps it out of the payload, so seeing one is a failure. Check that salons sharing name only show the name and nothing else. End with a section listing what to report as SKIP if unimplemented.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/tests/agent/walkthrough/05-compass-network-accounts.md`
...
1. `/home/danman60/projects/uvalux-platform/tests/agent/walkthrough/05-compass-network-accounts.md` exists and is complete.
2. `bash -c 'test -s /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/05-compass-network-accounts.md && grep -q "^## " /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/05-compass-network-accounts.md' && PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.vocab /home/danman60/projects/uvalux-platform/tests/agent/compass-knowledge.md /home/danman60/projects/uvalux-platform/tests/agent/walkthrough/05-compass-network-accounts.md` passes with exit code 0.
