# knowledge: ReviewStateChip

## What to build

A small status chip showing a claim's review state. Takes a single prop, the ReviewState from the contract. Render the human label from REVIEW_STATE_LABEL, never the raw enum value. Follow StatusChip in the exemplar exactly for markup shape and class naming: a span with a cp- prefixed class plus a state modifier class. Add no colours in the component; the modifier class carries them. Four states exist and all four need a modifier class.
...
1. `/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ReviewStateChip.tsx` exists and exports the component/function described above.
2. It imports what it uses from `@/lib/contract`.
3. `npx tsc --noEmit && python3 -m broker.vocab /home/danman60/projects/uvalux-platform/apps/web/src/components/compass/primitives.tsx /home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ReviewStateChip.tsx --contract /home/danman60/projects/uvalux-platform/packages/core/src/knowledge/curation/types.ts` passes with exit code 0.
