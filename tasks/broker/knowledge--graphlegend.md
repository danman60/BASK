# knowledge: GraphLegend

## What to build

A static legend explaining what the 3D graph's visual channels mean, so the graph is readable without training. Four rows: node colour means review state, node size means how many separate recordings said it, node brightness means how confident the provenance is, and a halo means the node has an open alert. Presentational only, no props, no state. Compact, sits over a dark canvas, so it needs its own cp- prefixed container class.
...
1. `/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/GraphLegend.tsx` exists and exports the component/function described above.
