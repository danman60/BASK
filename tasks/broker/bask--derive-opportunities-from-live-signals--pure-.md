# bask: derive opportunities from live signals (pure)

## What to build

Create a PURE module — no IO, no database, no imports outside the contract — that turns a salon's already-computed numbers into the Opportunity cards Today renders. First declare and export an interface named OpportunitySignals with EXACTLY these fields and no others: attachmentRatePercent: number; attachmentBaselinePercent: number; visitsLast14: number; averageSaleValue: number; currency: string; quietWeekdayHours: Array<{ weekday: number; hour: number; utilizationPercent: number }>; membershipEligibleCount: number; membershipConversionPercent: number; lapsed30Count: number; lowStock: Array<{ productName: string; daysOfCover: number }>. Then export one function deriveOpportunities(signals: OpportunitySignals): Opportunity[] which returns objects satisfying the Opportunity interface from the contract EXACTLY — every required field present, category/confidence/urgency values taken only from the exported const arrays, never a string literal you invented. Produce at most one opportunity per signal that clears a floor: retail attachment only when attachmentRatePercent is below attachmentBaselinePercent, quiet-hours only when a listed hour is under 50 percent utilization, memberships only when membershipEligibleCount is 5 or more, win-back only when lapsed30Count is 10 or more, reorder only when a lowStock entry has daysOfCover under 14. Sort the result by impactMonthly descending. impactMonthly is a plain number of dollars per month computed from the signals; impactLabel is that number formatted like '+$740/mo' with no decimals. whatChanged and whyItMatters are ONE short sentence each at a grade-7 reading level, stating only what the numbers say. confidenceNote states in one sentence why the confidence value was chosen. Do NOT set methodSource at all — attribution comes from a registry this module must not import. Do NOT invent a discount, a date, a person, or a product name that is not in the signals. Do NOT read process.env or the network.
...
1. `packages/core/src/opportunities/derive.ts` exists and is complete.
2. It imports what it uses from `./types`.
3. `PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.tscgate packages/core/src/opportunities/derive.ts --repo /home/danman60/projects/uvalux-platform --cmd 'npx tsc --noEmit' && PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.vocab /home/danman60/projects/uvalux-platform/packages/core/src/opportunities/fixtures.ts packages/core/src/opportunities/derive.ts --contract /home/danman60/projects/uvalux-platform/packages/core/src/opportunities/types.ts` passes with exit code 0.
