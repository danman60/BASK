# knowledge: KnowledgeEmpty

## What to build

A small empty-and-error state component. Props: title string, body string, and an optional error string. Render the title in an h3 and the body in a p. When error is present also render it inside a pre element with className cp-error-block so a real database message stays readable - never hide it. Wrap in a div with className cp-empty. Mirror CompassEmpty in the exemplar closely; this exists because the knowledge surface must show a raw error and CompassEmpty takes none.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/KnowledgeEmpty.tsx`
