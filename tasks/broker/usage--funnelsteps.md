...
A vertical stepped funnel showing drop-off between stages. Props: an array of steps, each with a name and a count. Render each step as a horizontal bar whose width is proportional to the first step's count, with the absolute count and the percentage of the first step. Between consecutive steps show the drop-off as a number and a percentage. Pure inline SVG or divs with token colours - do not import any charting library. When the array is empty render an empty state sentence.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/usage/FunnelSteps.tsx`
