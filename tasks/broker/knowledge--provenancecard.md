# knowledge: ProvenanceCard

## What to build

Displays one ClaimProvenance from the contract as the trust anchor of the whole surface. The verbatim quote is the visual hero, rendered as a blockquote and never truncated. Beneath it, subordinate, show the event label, the knowledgeRef, the timecode range formatted with formatTimecode from the contract, the source file basename and the audio stream index. When sessionTitle is null say plainly that no session could be matched. When speaker is present show it together with the titleConfidence disclosure. Take an optional onPlay callback and render a play control that calls it; when onPlay is absent render text saying the audio is not reachable from here, never a dead button.
...
Everything below is REAL and already exists. Import from `@/lib/contract`.
...
- Do not modify the contract, the exemplar, or anything outside the target path.
...
- Do not leave TODOs, stubs, or placeholder components.
...
compiler is a failure, not a completion.
