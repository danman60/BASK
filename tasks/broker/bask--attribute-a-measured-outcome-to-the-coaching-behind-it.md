# bask: attribute a measured outcome to the coaching behind it (pure)

## What to build

Create a PURE module — no IO, no database, no React, no imports outside the contract — that answers 'this campaign ran, it moved a number, and here is the coaching it came from'. Declare and export an interface named SettledCampaign with EXACTLY these fields: campaignId: string; name: string; goal: string; sentAt: string; audienceReached: number; metricLabel: string; metricBefore: number; metricAfter: number; unit: 'percent' | 'currency' | 'count'; currency: string; coaching: ClaimCitation[]. Then export attributeOutcome(campaign: SettledCampaign): OutcomeAttribution | null, plus an exported OutcomeAttribution interface holding: campaignId; headline: string; deltaLabel: string; direction: 'up' | 'down' | 'flat'; movedBy: number; sentenceOne: string; coaching: ClaimCitation[]. Return null when audienceReached is 0 or when metricBefore and metricAfter are equal — an outcome with nothing to report must not render a card. movedBy is metricAfter minus metricBefore. deltaLabel formats that difference by unit: percent as '+1.4 points' or '-1.4 points', currency as '+$740' with no decimals, count as '+12'. direction is up when movedBy is positive, down when negative, flat otherwise. sentenceOne is ONE grade-7 sentence naming the metric, the movement and the audience size, and NOTHING else — it must never claim the campaign caused the movement, only that both happened; use wording like 'went out to 84 people, and X moved from A to B'. Carry the coaching array through UNCHANGED — do not reword a claim, do not drop the quote, do not sort it, and never read or expose any field that is not on ClaimCitation. Do NOT invent a percentage, a date, or a name that is not on the input.

## Target file — write EXACTLY this path, and nothing else

`packages/core/src/opportunities/attribution.ts`

## The API surface you may use

Everything below is REAL and already exists. Import from `../knowledge/retrieve-claims`.
...
functions:
  toClaimCitation(match: ClaimMatch): ClaimCitation
  dedupeClaims(citations: readonly ClaimCitation[]): ClaimCitation[]

consts: CLAIM_SOURCE_LABEL
types: ClaimQueryFn
interfaces: ClaimMatch, ClaimCitation
```
...
1. `packages/core/src/opportunities/attribution.ts` exists and is complete.
2. It imports what it uses from `../knowledge/retrieve-claims`.
3. `PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.tscgate packages/core/src/opportunities/attribution.ts --repo /home/danman60/projects/uvalux-platform --cmd 'npx tsc --noEmit' && PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.vocab /home/danman60/projects/uvalux-platform/packages/core/src/network/outcomes.ts packages/core/src/opportunities/attribution.ts --contract /home/danman60/projects/uvalux-platform/packages/core/src/knowledge/retrieve-claims.ts` passes with exit code 0.
