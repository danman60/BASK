# walk: Inventory + Marketing

## What to build

Write a markdown QA checklist section following the exemplar's structure for three Bask screens: slash inventory, slash inventory slash order, and slash marketing. For inventory check stock levels render, that low stock or reorder signals are visible, and that any suggested order quantity shows the reasoning. For the order screen check a draft order can be opened and that it never submits anything real during a test run - state this as a hard rule in the checklist. For marketing check the campaign builder renders and that opening an existing campaign from a list actually rehydrates its content rather than showing an empty body. End with a section listing what to report as SKIP if unimplemented.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/tests/agent/walkthrough/03-inventory-marketing.md`
...
1. `/home/danman60/projects/uvalux-platform/tests/agent/walkthrough/03-inventory-marketing.md` exists and is complete.
