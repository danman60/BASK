...
A single row in the claims table. Import Claim, REVIEW_STATE_LABEL and formatTimecode from the package @bask/core. Props: claim, focused boolean, onSelect, onVerify, onReject. Render a tr with className cp-claim-row, adding cp-claim-row--focused when focused, and onClick calling onSelect. Cells in order: review state label, the claim text wrapped in a span with className cp-claim-text, the category, the moment, the distinctEvents number, and formatTimecode of the first provenance entry tStart. Guard against an empty provenance array. Do not render the quote. No colours in the component.
