# bask-insight-materiality-config

## What to build

Create a small, pure configuration module that names the materiality thresholds the insight detectors should use, expressed as MaterialityRule objects from the contract. It must export exactly two frozen constants and nothing else. First, a rule for judging a fall in retail attachment rate: absolute threshold of 3 percentage points, relative threshold of 0.40 (a forty percent loss of the baseline). Second, a rule for judging whether one staff member sits far enough below the house attachment rate to be named: absolute threshold of 6 percentage points, relative threshold of 0.30. These exact numbers are given, do not derive or adjust them. The reason both arms exist: on the synthetic demo fixtures attachment runs about 21 percent where the absolute arm is the meaningful test, and on the real twelve-year dataset attachment is 5.28 percent where a 3-point absolute drop would be a 57 percent collapse and would almost never fire, so the relative arm is the meaningful test there. The 0.30 gap figure is chosen because the measured 2019 staff spread was 3.37 percent against a 5.28 percent house rate, a shortfall of 0.362, so a 0.30 relative rule names that staffer while an absolute-only rule arithmetically never can. Each constant needs a doc comment explaining which detector consumes it and why both arms are present. This module holds configuration only: no functions, no detector logic, no imports beyond the MaterialityRule type from the contract. Match the file-header comment style and the tone of the contract file.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/packages/core/src/insights/thresholds.ts`
