# promo v6: FigurePlate — a framed cutout with a camera move on it

## What to build

Create a Remotion presentational component for the promo film: ONE element cutout from the app, framed as a figure with a slow camera move on it, so the cut composes shots instead of scrolling a page. Export an interface FigurePlateProps with EXACTLY: src: string; caption?: string; move?: 'push' | 'drift' | 'settle'; delay?: number. Export a function component FigurePlate. Behaviour, all driven by useCurrentFrame and interpolate from remotion, never CSS animation: (1) the image is rendered with the Img component and staticFile from remotion, contained not cropped, centred, with a soft rounded frame and a shadow taken from the token module; (2) it fades in over 12 frames starting at the delay prop, which defaults to 0; (3) the move prop selects the motion — 'push' scales from 1.0 to 1.06 across 90 frames, 'drift' translates 22 pixels horizontally across 90 frames while holding scale, 'settle' scales from 1.04 down to 1.0 over 30 frames and then holds. Default is 'push'; (4) when caption is provided it fades in 10 frames after the image and sits beneath the figure using the body font from the contract. Every colour, easing, shadow and font family comes from the contract token module. Add a header comment stating that this exists because 48 element cutouts already sit in promo/public/textures/v5 and the v5 cut framed almost none of them. Do NOT invent hex colours, do NOT import outside remotion, react and the contract, do NOT add a click handler, and do NOT hardcode any file path — the caller passes src.

## Target file — write EXACTLY this path, and nothing else

`promo/src/shots/v6/FigurePlate.tsx`
...
1. `promo/src/shots/v6/FigurePlate.tsx` exists and is complete.
2. It imports what it uses from `../../tokens`.
3. `npx tsc --noEmit && PYTHONPATH=/home/danman60/projects/sysadmin python3 -m broker.vocab /home/danman60/projects/uvalux-platform/promo/src/shots/v5/Plate.tsx promo/src/shots/v6/FigurePlate.tsx --contract /home/danman60/projects/uvalux-platform/promo/src/tokens.ts` passes with exit code 0.
