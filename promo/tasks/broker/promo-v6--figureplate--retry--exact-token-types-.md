# promo v6: FigurePlate (retry, exact token types)

## What to build

Create a Remotion presentational component. Export an interface FigurePlateProps with EXACTLY these fields: src: string; caption?: string; move?: 'push' | 'drift' | 'settle'; delay?: number. Export a function component named FigurePlate. ONE element cutout from the app framed as a figure with a slow camera move. Render the image with the Img component and staticFile from remotion, contained not cropped, centred, rounded frame. It fades in over 12 frames starting at the delay prop which defaults to 0. The move prop selects motion: push scales 1.0 to 1.06 across 90 frames, drift translates 22 pixels horizontally across 90 frames holding scale, settle scales 1.04 down to 1.0 over 30 frames then holds. Default push. When caption is provided it fades in 10 frames after the image and sits beneath the figure. All motion is driven by useCurrentFrame and interpolate imported from remotion — never CSS animation, never setInterval.
...
Add a header comment saying 48 element cutouts already sit in promo/public/textures/v5 and the v5 cut framed almost none of them. Do not invent hex colours, do not import anything outside remotion, react and the contract module, and do not hardcode a file path.
