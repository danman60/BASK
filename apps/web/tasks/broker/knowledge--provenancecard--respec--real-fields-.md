# knowledge: ProvenanceCard (respec, real fields)

## What to build

WHAT THIS IS FOR: a reviewer is looking at a Claim and needs to know whether to trust where it came from. Show the provenance so they can judge it at a glance. Take a prop of type ClaimProvenance. Use ONLY these real fields: event and sessionTitle for what it was, knowledgeRef for the human reference string, speaker for who said it, tStart and tEnd for the span (render them through formatTimecode, which takes seconds), titleConfidence, and quoteVerified. There are no fields called transcript or startTime; the previous attempt invented both. Two things MUST be visible because they are trust signals: when titleConfidence is 'interpolated' say the speaker was inferred from the agenda rather than heard, and when quoteVerified is false say the quote no longer matches its transcript. When sessionTitle or speaker is null, say so plainly - never invent a name. Style it from the exemplar's vocabulary only: cp-chip, cp-dot, cp-note, cp-statrow, cp-ev, cp-ev-item, cp-empty and their modifiers. The previous attempt invented cp-card, cp-card-header, cp-card-body and cp-provenance, none of which exist.
