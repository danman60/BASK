# knowledge: ProvenanceCard (respec 2, real chip modifiers)

## What to build

WHAT THIS IS FOR: a reviewer looking at a Claim needs to judge, at a glance, whether to trust where it came from. Take a prop of type ClaimProvenance. Show event, sessionTitle, knowledgeRef, speaker, and the span tStart-tEnd rendered through formatTimecode (it takes seconds). Two trust signals MUST be visible: when titleConfidence is 'interpolated', say the speaker was inferred from the agenda rather than heard; when quoteVerified is false, say the quote no longer matches its transcript. When sessionTitle or speaker is null say so plainly and never invent a name. CLASS NAMES - use ONLY these, they are the only ones that exist: cp-statrow with inner 'l' and 'v' spans for each labelled row, cp-note for a quiet explanatory line, cp-empty for a null value, and cp-chip for a status pill. The chip has exactly four modifiers: cp-chip--steady, cp-chip--watch, cp-chip--grow, cp-chip--order. Use cp-chip--steady for the good state (anchored speaker, verified quote) and cp-chip--watch for the state needing attention (interpolated speaker, unverified quote). The last attempt invented cp-chip--inferred, cp-chip--verified and cp-chip--unverified; those do not exist and fail the lint.

## Target file — write EXACTLY this path, and nothing else

`/home/danman60/projects/uvalux-platform/apps/web/src/components/compass/knowledge/ProvenanceCard.tsx`
