# knowledge: AlertChip (rebuild, fixed import repair)

## What to build

A small chip labelling a CurationAlert. Take the alert's kind (AlertKind) and severity (AlertSeverity) and render the human label from ALERT_LABEL. Import those three from @bask/core and do not redeclare any of them locally. Style with the exemplar's chip vocabulary: cp-chip, plus cp-chip--watch for a severity needing attention and cp-chip--steady otherwise. Use no other class tokens.
